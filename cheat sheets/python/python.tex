...
\node [mybox] (box){%
	...
	\begin{center}\small{\begin{tabular}{lp{4cm} l}
		\hline
		Explicit start/end \qquad\enspace\enspace\;\; &
		\begin{minipage}[t]{\textwidth}
		\begin{pythoncode}
		a[2:4]  # ['c','d']
		\end{pythoncode}
...
\node [mybox] (box){%
	\begin{minipage}{0.3\textwidth}
	Syntax: \\
	\scriptsize{\texttt{[expr(v) for v in some\_list (if predicate(v))]}} \\
...
\node[fancytitle, right=10pt] at (box.north west) {List comprehensions};
\end{tikzpicture}
...
\node [mybox] (box){%
	\begin{minipage}{0.3\textwidth}
	Syntax: \\
	\scriptsize{\texttt{\{expr(k):expr(v) for (k, v) in zip(l1, l2) (if predicate(k, v))\}}} \\
	\scriptsize{\texttt{\{expr(k):expr(k) for k in some\_list (if predicate(k))\}}} \\
...
\node[fancytitle, right=10pt] at (box.north west) {Dict comprehensions};
\end{tikzpicture}
